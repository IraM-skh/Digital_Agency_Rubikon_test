define(["jquery"], function ($) {
  return function () {
    const self = this;
    //Свой код я добавила вверх скрипта, чтобы было проще проверять задание.

    //Изменение сообщения о статусе загрузки
    const copyLeadSuccessStatus = () => {
      $(".copy-lead__info")
        .removeClass("copy-lead__info_load copy-lead__info_error")
        .addClass("copy-lead__info_success")
        .text("Готово!");
    };
    const copyLeadErrorStatus = () => {
      $(".copy-lead__info")
        .removeClass("copy-lead__info_load copy-lead__info_success")
        .addClass("copy-lead__info_error")
        .text("Ошибка");
    };
    const copyLeadLoadStatus = () => {
      $(".copy-lead__info")
        .removeClass("copy-lead__info_success copy-lead__info_error")
        .addClass("copy-lead__info_load")
        .text("Загрузка...");
    };

    //Валидация полей, которые необходимы для тела POST.
    //Не нашла в документации url, откуда можно было бы запросить названия полей для отправки данных на сервер или указать флаг для явного игнорирования невалидных полей. Пришлось захардкодить значения.
    const validLeadDataForSending = (data, selectedPipelineAndStatus) => {
      const validParams = [
        "name",
        "price",
        "created_by",
      ...
      const validParams_embedded = ["tags", "contacts", "companies"];
      //Будущий объект для отправки
      const objForSend = {
        pipeline_id: selectedPipelineAndStatus.pipeline_id,
        status_id: selectedPipelineAndStatus.status_id,
      };
      //Валидирую ключи прочитанного объекта
      function filerParam(paramArray, newObject, prevObject) {
        const prevObjectKeys = Object.keys(prevObject);
        paramArray.forEach((param) => {
          if (prevObjectKeys.includes(param) && prevObject[param]) {
            newObject[param] = prevObject[param];
          }
        });
      }
      filerParam(validParams, objForSend, data);
      //Добавляю свойства объекта _embedded, если он задан
      if (data._embedded) {
        objForSend._embedded = {};
        validParams_embedded.forEach((param) => {
          const path = data._embedded[param][0];
          if (path) {
            objForSend._embedded[param] = [{}];
            if (path["id"]) {
              objForSend._embedded[param][0].id = path["id"];
            }
            if (path["is_main"]) {
              objForSend._embedded[param][0].is_main = path["is_main"];
            }
          }
        });
        //Если не было добавлено валидных свойств, удаляю свойство _embedded
        if (Object.keys(objForSend._embedded).length < 1) {
          delete objForSend._embedded;
        }
      }
      return objForSend;
    };

    //Копирование сделки, вывод скопированных данных сделки в консоль
    const copyLead = (dataForSend) => {
      $.ajax({
        method: "POST",
        url: "/api/v4/leads",
        dataType: "json",
        ...
        success: function (data) {
          console.log("Ответ сервера на успешное копирование", data);
          copyLeadSuccessStatus();
        },
        error: function () {
          copyLeadErrorStatus();
        },
      });
      console.log("Данные скопированной сделки:", dataForSend);
    };

    //Чтение сделки, вывод данных сделки в консоль
    const readingLead = (leadId, selectedPipelineAndStatus) => {
      $.ajax({
        method: "GET",
        ...
        success: function (data) {
          console.log(`Прочитанная сделка:`, data);
          copyLead([validLeadDataForSending(data, selectedPipelineAndStatus)]);
        },
        error: function () {
          copyLeadErrorStatus;
        },
      ...
      render: function () {
        ...
        $(".copy-lead__button").on("click", function () {
          if (!$(this).hasClass("copy-lead__button_disable")) {
            ...
            //Копирование сделки
            readingLead(lead_id, { pipeline_id, status_id });
          }
        });
